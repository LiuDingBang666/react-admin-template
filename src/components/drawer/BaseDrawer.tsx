      ...
      {props.record && (
        <>
          <Space
            direction="vertical"
            size="middle"
            style={{ display: 'flex', marginBottom: 15, fontWeight: 'bold' }}
          >
            <Row justify={'center'}>
              <Col span={12}>
                <div>
                  <UserAddOutlined /> 创建人: {props.record.createdBy ?? '-'}
                </div>
              </Col>
              <Col span={12}>
                <div>
                  <ClockCircleOutlined /> 创建时间: {props.record.createdAt ?? '-'}
                </div>
              </Col>
            </Row>
            <Row justify={'center'}>
              <Col span={12}>
                <div>
                  <UsergroupAddOutlined /> 修改人: {props.record.updatedBy ?? '-'}
                </div>
              </Col>
              <Col span={12}>
                <div>
                  <ClockCircleOutlined /> 修改时间: {props.record.updatedAt ?? '-'}
                </div>
              </Col>
            </Row>
          </Space>
